...
import traceback
import io, os, re, textwrap, concurrent.futures
import numpy as np
import cv2
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                try:
                    ...
                    print(f"🔥 Translation thread crashed on {fname}: {e}")
                    traceback.print_exc()

        return fname, image, page_log

    except Exception as e:
        print(f"🔥 Fatal error in process_page ({fname}): {e}")
        traceback.print_exc()
        return fname, Image.new("RGB", (500, 500), "red"), [{"original": "ERROR", "cleaned": "", "translated": str(e)}]

# ==============================
# Streamlit UI
# ==============================
st.title("🚀 Bubble-Aware Manhua Translator (Debug Mode)")

uploaded_files = st.file_uploader("Upload Images or PDF", type=["jpg","png","jpeg","pdf"], accept_multiple_files=True)
    ...
    try:
        ...
        if total_files == 0:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                futures = [executor.submit(process_page, fname, fbuf, reader, mode) for fname, fbuf in all_files]
                for idx, future in enumerate(concurrent.futures.as_completed(futures), start=1):
                    try:
                        ...
                        st.error(f"🔥 Worker crashed on {fname}: {e}")
